class Web::DashboardController < WebApplicationController
  ...
  def index
    ...
    @today_reservations = current_user.reservations.joins(:schedule).where(schedules: { weekday: Date.today.wday }).count
    ...
    @completed_reservations = current_user.reservations.confirmed.where("created_at < ?", 1.month.ago).count
    # .joins(schedule: :course) - joins the tables: reservations + schedules + courses. This gives access to the course price.
    @total_spent = current_user.reservations.confirmed.joins(schedule: :course).sum("courses.price").to_f
    #--------------------------------------------------------------------
